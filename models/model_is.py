...
def imune_parameters(caso=0):

    # caso 0: sobrevivente
    # caso 1: nao-sobrevivente

    # parametros sistema imune
    pars = {}
    if(caso == 0):
        pars['pi_v']     = 1.47
    elif(caso == 1):
        pars['pi_v']     = 1.4742055035646986
    ...
    if(caso==0):
        pars['beta_apm'] = 1.33e-2 #sobreviventes
    elif(caso==1):
        pars['beta_apm'] = 1.51e-2 #nao sobreviventes

    if(caso==0):
        pars['beta_tke'] = 3.5e-6 #sobreviventes  
    elif(caso==1):
        pars['beta_tke'] = 1.0e-6 #nao sobreviventes
    ...
    if(caso==0):
        pars['alpha_b']  = 3.578236584 # correcao Ruy
    elif(caso==1):
        pars['alpha_b']  = 3.578236584371140339e+2
    ...
    if(caso==0):
        pars['pi_ci'] = 6.44e-3 #sobreviventes
    elif(caso==1):
        pars['pi_ci'] = 9.96e-2 #nao sobreviventes
...
def solve_imune(pars,ts,y0):
    sol = solve_ivp(modelo_is, ts, y0, args=(pars,) )#method='Radau')
    return sol
...
if __name__ == "__main__":
    ...
    sol = solve_imune(pars,tspan,y0_imune)
    ...
    plt.subplot(2,2,1)
    plt.plot(t,np.log10(V+1), color='red', label='V')
    plt.xlabel('Tempo [dias]')
    plt.ylabel('Viremia $\\log_{10}$[cópias/mL]')
    plt.legend()
    plt.grid()
    ...
    plt.xlabel('Tempo [dias]')
    plt.ylabel('Citocinas [pg/mL]')
    plt.legend()
    plt.grid()

    plt.subplot(2,2,3)
    plt.plot(t,np.log2(IgG+1), color='orange', label='IgG')
    plt.xlabel('Tempo [dias]')
    plt.ylabel('IgG $\\log_{2}$[S/CO]')
    plt.legend()
    plt.grid()

    plt.subplot(2,2,4)
    plt.plot(t,np.log2(IgM+1), color='black', label='IgM')
    plt.xlabel('Tempo [dias]')
    plt.ylabel('IgM $\\log_{2}$[S/CO]')
    plt.legend()
    plt.grid()
